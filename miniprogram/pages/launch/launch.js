...
const funPrizeNames = ['旅行随行杯', '电影兑换券', '幸运零食盒', '桌面小风扇', '咖啡体验券', '惊喜红包'];
...
const conditionGroups = [
  {
    title: '参与后任务',
    items: [
      { key: 'checkIn', title: '设置打卡任务', linkText: '示例' },
...
Page({
  data: {
    ...
    isFun: false,
    isUnlock: false,
    launchButtonText: '发起抽奖',
    annualConfig: {
      companyName: '',
      activityTheme: '年会抽奖',
      brandLogo: '',
      backgroundImage: '',
      showWinnerContact: false,
      candidateUnionIds: '',
      onlyWecom: false
    },
    activePrizeIndex: 0,
    ...
    conditionConfig: {
      ...
      assistLimit: '5',
      checkInTitle: '完成限时打卡',
      checkInDurationSeconds: '15',
      checkInGuideText: '完成打卡后，获得额外抽奖权重',
  ...
  onLoad(options = {}) {
    ...
    this.setData({ standalone: options.standalone === '1' });
    if (options.preset) {
      this.applyTemplatePreset(options.preset, options.style || '');
  ...
  selectTemplate(event) {
    this.applyTemplatePreset(event.currentTarget.dataset.key);
  },
  applyTemplatePreset(templateKey, styleKey = '') {
    ...
    let prizes = this.persistActivePrize();
    let activePrizeIndex = this.data.activePrizeIndex;
    let activePrize = this.data.activePrize;
    let prizeTypeTabs = this.data.prizeTypeTabs;
    if (template.key === 'unlock') {
      activeDrawMode = 'time';
      unlockByPeople = true;
      if (prizes.length < 2) {
        prizes = [createPrize(0), createPrize(1)];
        prizes[0].unlockParticipants = '3';
        activePrizeIndex = 0;
        activePrize = { ...prizes[0] };
        prizeTypeTabs = buildPrizeTabs(activePrize.type);
      }
    }
    if (template.key === 'fun') {
      const randomName = funPrizeNames[Math.floor(Math.random() * funPrizeNames.length)];
      prizes = [{ ...createPrize(0), name: randomName, image: '' }];
      activePrizeIndex = 0;
      activePrize = { ...prizes[0] };
      prizeTypeTabs = buildPrizeTabs(activePrize.type).filter(item => item.title === '奖品' || item.title === '红包');
    }
    ...
    this.setData({
      ...
      templateType: styleKey && styleNames[styleKey] ? styleNames[styleKey] : template.title,
      templateOptions: buildTemplateOptions(template.key),
      pageTitle: template.key === 'annual' || template.key === 'fun' ? '抽奖助手' : '发起抽奖',
      sectionLabel: template.key === 'annual'
        ? '年会抽奖（开奖时按倒序抽奖）'
        : (template.key === 'fun' ? '趣味抽奖' : '普通抽奖'),
      typeIntroText: template.key === 'wechat'
        ? '微信群的抽奖玩法介绍'
        : (template.key === 'official' ? '公众号传播、涨粉的抽奖玩法介绍' : ''),
      styleKey,
      styleName: styleNames[styleKey] || '',
      isAnnual: template.key === 'annual',
      isFun: template.key === 'fun',
      isUnlock: template.key === 'unlock',
      launchButtonText: template.key === 'annual' ? '发起年会抽奖' : (template.key === 'fun' ? '发起新抽奖' : '发起抽奖'),
      prizes,
      activePrizeIndex,
      activePrize,
      prizeTypeTabs,
  ...
  onSpecialInput(event) {
    ...
    this.setData({ [`annualConfig.${field}`]: event.detail.value });
  },
  async chooseAnnualImage(event) {
    if (this.data.uploading) return;
    const field = event.currentTarget.dataset.field;
    if (!['brandLogo', 'backgroundImage'].includes(field)) return;
    try {
      ...
      this.setData({ [`annualConfig.${field}`]: assetUrl(url) });
      wx.hideLoading();
  ...
  toggleAnnualOption(event) {
    const field = event.currentTarget.dataset.field;
    const checked = Boolean(event.detail.value);
    const updates = { [`annualConfig.${field}`]: checked };
    if (field === 'onlyWecom') {
      const conditionValues = { ...this.data.conditionValues, wecom: checked };
      updates.conditionValues = conditionValues;
      updates.conditionGroups = buildConditionGroups(conditionValues);
    }
    this.setData(updates);
  },
  editCandidateUnionIds() {
    wx.showModal({
      title: '设置指定身份名单',
      editable: true,
      placeholderText: '输入微信 UnionID，多个标识用逗号分隔',
      content: this.data.annualConfig.candidateUnionIds,
      success: result => {
        if (result.confirm) this.setData({ 'annualConfig.candidateUnionIds': String(result.content || '').trim() });
      }
    });
  },
  randomizeFunPrize() {
    const current = String(this.data.activePrize.name || '');
    const options = funPrizeNames.filter(item => item !== current);
    const name = options[Math.floor(Math.random() * options.length)] || funPrizeNames[0];
    this.setData({ 'activePrize.name': name });
  },
  persistActivePrize() {
  ...
  selectDrawMode(event) {
    ...
    const conditionValues = activeDrawMode === 'instant'
      ? { ...this.data.conditionValues, assist: false, checkIn: false }
      : this.data.conditionValues;
    this.setData({
      activeDrawMode,
      drawModeTabs: buildDrawTabs(activeDrawMode),
  ...
  onTaskProofRequirementChange(event) {
    this.setData({ 'conditionConfig.taskProofRequired': Boolean(event.detail.value) });
  },
  onCheckInRewardModeChange(event) {
    const checkInRewardMode = event.detail.value === 'random' ? 'random' : 'fixed';
    const updates = { 'conditionConfig.checkInRewardMode': checkInRewardMode };
    if (checkInRewardMode === 'fixed') {
      updates['conditionConfig.checkInRewardMax'] = this.data.conditionConfig.checkInRewardMin;
    }
    this.setData(updates);
  },
  ...
  toggleCondition(event) {
    ...
    if ((key === 'assist' || key === 'checkIn') && enabling && this.data.activeDrawMode === 'instant') {
      wx.showToast({ title: '即抽即中不支持助力或打卡加权', icon: 'none' });
      return;
    }
  ...
  async submit(event) {
    ...
    if (this.data.activeDrawMode === 'people' && drawParticipantTarget < prizeCount) {
      wx.showToast({ title: '开奖人数不能少于奖品总份数', icon: 'none' });
      return;
    }
    ...
    const assistWeight = Number(conditionConfig.assistWeight || 0);
    const assistLimit = Number(conditionConfig.assistLimit || 0);
    if (this.data.conditionValues.assist &&
      (!Number.isInteger(assistWeight) || assistWeight < 1 || assistWeight > 10 ||
        !Number.isInteger(assistLimit) || assistLimit < 1 || assistLimit > 50)) {
      wx.showToast({ title: '请设置有效的助力权重和助力上限', icon: 'none' });
      return;
    }
    const checkInTitle = String(conditionConfig.checkInTitle || '').trim();
    const checkInDurationSeconds = Number(conditionConfig.checkInDurationSeconds || 0);
    const checkInRewardMode = conditionConfig.checkInRewardMode === 'random' ? 'random' : 'fixed';
    const checkInRewardMin = Number(conditionConfig.checkInRewardMin || 0);
    const checkInRewardMax = checkInRewardMode === 'random'
      ? Number(conditionConfig.checkInRewardMax || 0)
      : checkInRewardMin;
    if (this.data.conditionValues.checkIn &&
      (checkInTitle.length < 2 || checkInTitle.length > 40 ||
        !Number.isInteger(checkInDurationSeconds) || checkInDurationSeconds < 3 || checkInDurationSeconds > 300 ||
        !String(conditionConfig.checkInGuideText || '').trim() ||
        !Number.isInteger(checkInRewardMin) || checkInRewardMin < 1 || checkInRewardMin > 10 ||
        !Number.isInteger(checkInRewardMax) || checkInRewardMax < checkInRewardMin || checkInRewardMax > 10)) {
      wx.showToast({ title: '请完整设置打卡任务、时长和权重奖励', icon: 'none' });
      return;
    }
    ...
    try {
      const activity = await request('/api/activities', {
        ...
        data: {
          ...
          specialConfig: {
            styleKey: this.data.styleKey,
            styleName: this.data.styleName,
            annualReverseDraw: this.data.isAnnual,
            annual: this.data.isAnnual ? {
              ...this.data.annualConfig,
              candidateUnionIds: String(this.data.annualConfig.candidateUnionIds || '')
                .split(/[，,]/)
                .map(item => item.trim())
                .filter(Boolean)
            } : null,
            fun: this.data.isFun ? { randomPrize: true } : null
          },
          ...
          conditions: {
            ...this.data.conditionValues,
            assistWeight: this.data.conditionValues.assist ? assistWeight : 1,
            assistLimit: this.data.conditionValues.assist ? assistLimit : 5,
            checkInTask: this.data.conditionValues.checkIn ? {
              type: 'timed',
              title: checkInTitle,
              durationSeconds: checkInDurationSeconds,
              guideText: String(conditionConfig.checkInGuideText || '').trim(),
              rewardMode: checkInRewardMode,
              rewardMin: checkInRewardMin,
              rewardMax: checkInRewardMax
            } : null,
            ...
            taskProofRequired: this.data.conditionValues.task ? Boolean(conditionConfig.taskProofRequired) : false,
